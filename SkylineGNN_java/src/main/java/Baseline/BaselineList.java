...
public class BaselineList implements Runnable {
    private final static int data_dimension = 3;

    private final static int SAVING = 1;
    private final static int DEBUGGING = 0;
...
//    long total_time = 0;
    ...
    public String methed_name = "gnn";
    public Neo4jDB neo4j;
    public List<Long> t_nodes;
    public int index;


    public BaselineList(String db_name) {
        this.db_name = db_name;
    }

    public BaselineList(String db_name, int number_queries, ArrayList<Pair<Long, Long>> query_list, boolean init, boolean lnd, int lnd_n) {
        this.db_name = db_name;
    ...
    public void init_landmark() {
        ...
        System.out.println("Read the landmark index from the folder :" + landmark_folder_str);
...
//                }
                ...
                for (int i = 0; i < v.skyPaths.size(); i++) {
                    ...
                    if (!p.expaned) {
                        ...
                        ArrayList<path> new_paths = p.expand(neo4j, t_nodes);
                        expansion_rt += (System.nanoTime() - st_expansion_rt);


                        for (path np : new_paths) {
                            if (!t_nodes.contains(np.endNode)) {
                                continue;
                            }
...
//        this.total_query_time += quer_running_time / 1000000;
//        this.total_time += (quer_running_time + addtoskyline_rt + check_dominate_result_rt + upperbound_find_rt + expansion_rt) / 1000000;
...
//        System.out.println("funciton "+(neo4j.graphDB == null) + "," + Thread.currentThread().isInterrupted()+","+Thread.currentThread().getName());
        ...
        if (DEBUGGING==1) {System.out.println("===========================initilizeSkylinePath()============================");}
        int i = 0;
        results.clear();
...
//                    this.iniLowerBound[i++] = paths.weight();
                    if (DEBUGGING==1) {
                        System.out.println("Found Weighted Paths by property_name [" + property_name + "]: ");
...
//        System.exit(0);
    ...
    @Override
    public void run() {
        System.out.println("===================Class BaselineList===========================");
        init_landmark();
        ArrayList<path> results = queryOnline(this.current_src, this.current_dest, this.t_nodes);
        System.out.println(this.index + ": The size of the final result is " + results.size());
//        System.out.println("Total query time: " + this.total_query_time + " ms");
//        System.out.println("Total time: " + this.total_time + " ms");
        String paths_output_file = result_folder + "/" + this.methed_name + "_" + current_src + "_" + current_dest + "_" + postfix + ".log";
        System.out.println("saved: " + paths_output_file);
        if (SAVING==1) {savePathsInformation(results, paths_output_file);}
        System.out.println("-----------------------------------------------------------------------------------------");
    }
}
