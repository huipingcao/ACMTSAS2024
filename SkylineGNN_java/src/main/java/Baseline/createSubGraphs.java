...
public class createSubGraphs {
    ...
    public String createChangeGraphDB(String dbname, String GraphInfoPath, int sub_size, int i) {

        String sub_db_name = dbname + "_" + sub_size + "K";
        String nodeFilePath = GraphInfoPath + "/NodeInfo_" + sub_size + "K_" + i + ".txt";
        String EdgeFilePath = GraphInfoPath + "/SegInfo_" + sub_size + "K_" + i + ".txt";


        String folder = ParsedOptions.neo4jdbPath + "/" + sub_db_name;

        System.out.println(nodeFilePath);
        System.out.println(EdgeFilePath);
    ...
    private void createRelation(String src, String des, double eDistence, double metersDistance, double runningTime, GraphDatabaseService graphdb) {
        try {
            Node srcNode = graphdb.getNodeById(Long.parseLong(src));
            Node desNode = graphdb.getNodeById(Long.parseLong(des));
