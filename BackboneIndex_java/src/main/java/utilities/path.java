...
//    public ArrayList<Long> rels;
    ...
    public path(myQueueNode current) {
...
//            this.nodes.add(r.getOtherNodeId(nodes.get(nodes.size() - 1)));
    ...
    /**
     * Expand current path from the end node of this path by only using the rels in current cluster and the deleted in current sub-step
     * @param neo4j the no4j object
     * @param de the deleted edges in current sub-step
     * @param cluster_rels the relationship in current cluster
     *
     * @return
     */
