...
public class myQueueNode {
    ...
    public myQueueNode(myQueueNode node, long current_id, Neo4jDB neo4j) {
        this.id = current_id;
    ...
    public void setLocations(myQueueNode start_node) {
        try (Transaction tx = neo4j.graphDB.beginTx()) {
            locations[0] = (double) neo4j.graphDB.getNodeById(this.id).getProperty("lat");
            locations[1] = (double) neo4j.graphDB.getNodeById(this.id).getProperty("log");
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof myQueueNode)) {
            return false;
        }

        // typecast o to Complex so that we can compare data members
        myQueueNode c = (myQueueNode) o;
