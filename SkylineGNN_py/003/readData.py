...
def LoadGraph(path, logger=None, normalization_node=True, normalization_edge=True):
    ...
    if debug:
        ...
        node_df.to_csv('./normalized_coor.csv')
...
def LoadData(path):
    ...
    train_paths_folder = path+"/training_GNN"
...
class pathObj():
    ...
    def __str__(self):
        return "{}->{}, {}, {}, {:.2f}".format(self.src, self.dest, len(self.nodes), self.getNumberPaths(),
                                               self.getAveragePathLength())


def readLogs(filename:str, t_i=-1):
    current_func = 'readLogs'
    ...
    with open(filename) as fp:
        ...
        for line in Lines:
            ...
            # print(nodes_in_path){current_file} - {current_func}: p_inst {p_inst}
            ...
            for n in nodes_in_path:
                nodes.add(n)
    if debug:
        print(f'{current_file} - {current_func}: nodes {nodes}')
        print(f'{current_file} - {current_func}: paths {paths[:10]}')
        print(f'{current_file} - {current_func}: costs {costs[:10]}')
    p_inst = pathObj(src, dest, nodes, paths, costs)
        # print(f'==================instance HERE!!======================')
        # print(f'{current_file} - {current_func}: p_inst {p_inst}')
    return p_inst
